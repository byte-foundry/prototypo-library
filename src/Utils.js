...
Utils.ufoToPaper = function (src) {
	...
	if (src.outline && src.outline.component) {
		...
		src.components.forEach((component) => {
			if (component.anchor) {
...
Utils.fontFromSrc = function (src) {
	...
	const font = new paper.Font(_.assign({}, src.fontinfo, {
	...
	const filteredSrc = _.assign({}, src);
...
Utils.glyphFromSrc = function (src, fontSrc, naive, embed) {
	...
	(glyph.src.anchors || []).forEach((anchorSrc) => {
		const anchor = new paper.Node();
	...
	(glyph.src.contours || []).forEach((contourSrc, contourIdx) => {
		...
		contourSrc.nodes.forEach((nodeSrc, nodeIdx) => {
			const node = new paper.Node();
	...
	glyph.embedComponents = function () {
		glyph.src.components.forEach((componentSrc) => {
...
Utils.selectGlyphComponent = function (
	...
	id,
	index) {
	const component = Utils.glyphFromSrc(
	...
	(componentSrc.parentAnchors || []).forEach((anchorSrc) => {
		const anchor = new paper.Node();
...
Utils.createUpdater = function (leaf/* , path*/) {
	let sOperation = leaf._operation.toString(),
	...
	Utils.updaterCache[cacheKey] = Function(...[
...
Utils.solveDependencyTree = function (leaf, src) {
	let depTree = Utils.dependencyTree(src || leaf.src, null),
		order = depTree.resolve().map(cursor => ({cursor: cursor.split('.')})),
...
Utils.dependencyTree = function (parentSrc, cursor, depTree) {
	...
	Object.keys(parentSrc).forEach((i) => {
		...
		if ((leafSrc._updaters && leafSrc._updaters.length)
			...
			depTree.add(currCursor,
				leafSrc._dependencies.filter(dep =>
...
Utils.simplifyResolutionOrder = function (leaf, depTree) {
	return depTree.filter((cursor) => {
		const src = Utils.propFromCursor(cursor.cursor, leaf.src);
...
Utils.updateParameters = function (leaf, params) {
	let paramsToUpdate = (leaf.src && leaf.src.parameters) || [];
...
Utils.updateIndividualParameters = function (leaf, params) {
	Object.keys((leaf.src && leaf.src.parameters) || [])
		.forEach((name) => {
			const src = leaf.src.parameters[name];
			...
			if (params.indiv_group_param) {
				Object.keys(params.indiv_group_param)
					.forEach((groupName) => {
						...
						if (src._parameters) {
							src._parameters.forEach((parameter) => {
...
Utils.updateProperties = function (leaf, params, erroredPreviously) {
	...
	const cursorKeys = params.manualChanges && params.manualChanges.cursors ? Object.keys(params.manualChanges.cursors) : [];

	if (cursorKeys.length > 0) {
		for (i = 0; i < cursorKeys.length; i++) {
			cursor = cursorKeys[i].split('.');
			const tmpObj = Utils.propFromCursor(cursor, leaf, cursor.length - 1);
			var tmpSrc;
...
Utils.updateXscenderProperties = function (font, params) {
	if (params.indiv_group_param) {
		...
		xscenderProperties.forEach((name) => {
			const src = font.src.fontinfo[name];
				...
				.forEach((groupName) => {
					...
					Math.abs(src._updaters[0].apply(
						...
						[
							...
							(src._parameters || []).map(_name => group[_name] || params[_name]),
						),
					)),
				);
				});
