...
function autoExpandableNodeSrc(node, i, j, inSkeleton) {
	return {
		...
		all: {_dependencies: Object.keys(node.src).map(key => Utils.cursor(i, j, key))},
...
function autoExpandedNodeSrc(node, i, j, side, isClosed) {
	return {
		...
		all: {
			_dependencies: Object.keys(node.src).map(key => Utils.cursor(i, j, key)).concat([
				Utils.cursor(i, j, 'expandedTo', side, 'point'),
			]),
			_updaters: [function () {
...
function explicitExpandableNodeSrc(node, i, j) {
	return {
		point: {_dependencies: []},
		all: {_dependencies: [0, 1].map(side => Utils.cursor(i, j, 'expandedTo', side, 'all'))},
		_dependencies: [],
	};
}
...
function explicitExpandedNodeSrc(node, i, j, side, isClosed) {
	return {
		...
		all: {_dependencies:
			Object.keys(node.src.expandedTo[side]).map(key => Utils.cursor(i, j, 'expandedTo', side, key)),
		},
		_dependencies: [
			Utils.cursor('contours', i, 'expandedTo',
				(isClosed ? side : 0), 'all'),
...
function expandedContourSrc(contour, i, side) {
	const half = contour.nodes.length / 2;

	return {
		all: {
			_dependencies: contour.nodes.map((node, j) => side !== undefined
			...
			_updaters: [function () {
				const curviness = arguments[arguments.length - 1];
...
function contourSrc(contour, i) {
	return {
		all: {
			_dependencies: contour.nodes.map((node, j) => Utils.cursor(i, j, 'all')),
			_parameters: ['curviness'],
			_updaters: [function () {
				const curviness = arguments[arguments.length - 1];
		...
		_dependencies: contour.nodes.map((node, j) => Utils.cursor(i, j)),
	};
}
	...
	glyph.contours.forEach((contour, i) => {
		...
		contour.nodes.forEach((node, j) => {
			...
			// the expanded node might have been defined explicitely
				...
				node.src.expandedTo.forEach((src, k) => {
...
naive.skeletonCopier = function (node) {
	let angle = node.expand && node.expand.angle || 0,
...
naive.updateContour = function (path, curviness) {
	...
	path.nodes.forEach((node) => {
		...
		if (rri === null) {
			let angle = Utils.lineAngle(start._point, end._point),
				middle = {
